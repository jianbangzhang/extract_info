...
from . import h26x_client, ne_utils
...
# 处理响应数据
# def deal_message(ws, message, dir_path):
...
def deal_message(ws, message,dir_path):
    temp_result = json.loads(message)
    code_msg="成功" if temp_result["header"]["code"]==0 else "失败"
    print("OCR响应数据状态:{}\n".format(code_msg))
    header = temp_result.get('header')
    if header is None:
        return
    code = header.get('code')
    if header is None or code != 0:
        print(f"OCR获取结果失败，CODE:{code},请根据code查证问题原因")
        return
    # print("sid:{}".format(header.get('sid')))
    status = header.get('status')
    # print('status:{}\n'.format(status))
        ...
        if len(response_match) > 0:
            for response_item in response_match:
                ...
                for media_type in media_type_list:
                    ...
                    real_data = base64.b64decode(media_value)
                    # response_path_split_list = response_path.split('.')
                    # file_name="{}.{}".format(response_path_split_list[len(response_path_split_list) - 1], encoding)
                    write_file_path = os.path.join(dir_path,"result.utf8")
                    with open(write_file_path, "ab") as file:
                        file.write(real_data)
